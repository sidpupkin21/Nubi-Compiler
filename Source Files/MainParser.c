...
* Function list: startParser(), startScanner(), 
*	printParserError(),displayParser(),getParserFileSize(),
*	callGarbageCollector(),mainParser()
************************************************************
*/
...
/* External objects */
extern Sint syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern Sint line; /* source code line number - defined in scanner.c */
...
extern Null startParser(void);
extern Sint startScanner(BufferPointer sc_buf);

static Null printParserError(char* fmt, ...);
static Null displayParser(BufferPointer ptrBuffer);
static Long getParserFileSize(char* fname);
static Null callGarbageCollector(void);
...
Null printParserError(chars* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(Null)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
Long getParserFileSize(chars* fname) {
	FILE* input;
	Long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
Null displayParser(Buffer* ptrBuffer) {
...
Null callGarbageCollector(Null) {
