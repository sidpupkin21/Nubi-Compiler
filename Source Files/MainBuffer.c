 ...
 * -------------------------------------------------------------
 */
Null bErrorPrint(chars* fmt, ...);
Null displayBuffer(Buffer* ptr_Buffer);
Long getFileSize(chars* fname);
Sint isNumber(const chars* ns);
Null startBuffer(chars*, chars*, chars, Sint, Sint);
...
Sint mainBuffer(Sint argc, chars** argv) {
	...
	chars* program = argv[0];
	chars* input = argv[2];
	chars mode = MODE_FIXED;
	Sint isAnsiC = !ANSI_C;		/* ANSI C flag */
	Sint size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
void startBuffer(chars* program, chars* input, chars mode, Sint size, Sint increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	Sint loadSize = 0;			/* the size of the file loaded in the buffer */
	chars symbol;				/* symbol read from input file */
